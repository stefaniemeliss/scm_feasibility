...
```{r base_chunk, echo=FALSE, eval=F}
### RAW DATA ###
...
out <- round(out, 3)
# print output
kbl(out, caption = paste0("Descriptives of variable ", dep_var, " RAW")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T) %>%
  print()
...
tmp[-1] %>% 
  filter(within_school_sd > 0 & within_school_mean != 0) %>%
  psych::describe(omit = T) %>%
  kbl(caption = "Descriptives of within-school statistics - RAW") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T) %>% 
  print()
...
df <- df %>%
  group_by(school_urn) %>%
  mutate(Diff = get(dep_var) - lag(get(dep_var))) %>%
  as.data.frame()

# look at histogram by year
plt <- ggplot(data = df, aes(x = Diff)) + 
  geom_histogram(bins = 25) + 
  facet_wrap(~get(group_var)) + 
  ambition_theme + 
  ylab("Count") +
  xlab(paste(y_lab, "- LAGGED"))
print(plt)

# compute descriptives per year
out <- rbind(
  do.call("rbind",psych::describeBy(df[, "Diff"], group = df[, group_var]))
)
# edit output
out$vars <- NULL
out <- round(out, 3)
# print output
kbl(out, caption = paste0("Descriptives of variable ", dep_var, " LAGGED")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T) %>% 
  print()
...
# plot overall time series
plt <- ggplot(data = df, aes(x = time_period, y = Diff, group = school_urn, col = school_urn)) +
  geom_point() + 
  geom_line() + 
  ambition_theme + theme(legend.position = "none",  
                         axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylab(paste(y_lab, "- LAGGED")) +
  xlab(x_lab)
print(plt)
...
# within school stats
tmp <- df %>% group_by(school_urn) %>%
  summarise(obs_per_school = sum(!is.na(Diff)),
            within_school_mean = mean(Diff, na.rm = T),
            within_school_sd = sd(Diff, na.rm = T),
            snr = abs(mean(Diff, na.rm = T))/sd(Diff, na.rm = T),
            rsd = sd(Diff, na.rm = T)/abs(mean(Diff, na.rm = T)))
tmp[-1] %>% 
  filter(within_school_sd > 0 & within_school_mean != 0) %>%
  psych::describe() %>%
  kbl(caption = "Descriptives of within-school statistics - LAGGED") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T) %>% 
  print()
...
plt <- ggplot(data = tmp, aes(x = rsd )) +
  ...
  theme(axis.title.y = element_blank(),
        ...
        axis.ticks.y = element_blank()) +
  scale_x_continuous(labels = scales::percent) +
  labs(caption = "Note. If mean values are small, %RSD will approach infinity.") +
  xlab(paste("%RSD", y_lab, "- LAGGED")) # % relative standard deviation
print(plt)

### TWO-YEAR ROLLING AVERAGE (SHIFTING WINDOW) ###

df <- df %>%
  group_by(school_urn) %>%
  mutate(Shift = zoo::rollapply(get(dep_var), width = 2, FUN = mean, align = 'right', fill = NA)) %>%
  as.data.frame()

# look at histogram by year
plt <- ggplot(data = df, aes(Shift)) + 
  geom_histogram(bins = 25) + 
  facet_wrap(~get(group_var)) + 
  ambition_theme + 
  ylab("Count") +
  xlab(paste(y_lab, "- 2Y ROLLING"))
print(plt)

# compute descriptives per year
out <- rbind(
  do.call("rbind",psych::describeBy(df[, "Shift"], group = df[, group_var]))
)
# edit output
out$vars <- NULL
out <- round(out, 3)
# print output
kbl(out, caption = paste0("Descriptives of variable ", dep_var, " 2Y ROLLING")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T) %>% 
  print()
...
# plot overall time series
plt <- ggplot(data = df, aes(x = time_period, y = Shift, group = school_urn, col = school_urn)) +
  geom_point() + 
  geom_line() + 
  ambition_theme + theme(legend.position = "none",  
                         axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylab(paste(y_lab, "- 2Y ROLLING")) +
  xlab(x_lab)
print(plt)
...
# within school stats
tmp <- df %>% group_by(school_urn) %>%
  summarise(obs_per_school = sum(!is.na(Shift)),
            within_school_mean = mean(Shift, na.rm = T),
            within_school_sd = sd(Shift, na.rm = T),
            snr = abs(mean(Shift, na.rm = T))/sd(Shift, na.rm = T),
            rsd = sd(Shift, na.rm = T)/abs(mean(Shift, na.rm = T)))
tmp[-1] %>% 
  filter(within_school_sd > 0 & within_school_mean != 0) %>%
  psych::describe() %>%
  kbl(caption = "Descriptives of within-school statistics - 2Y ROLLING") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T) %>% 
  print()
...
plt <- ggplot(data = tmp, aes(x = rsd )) +
  ...
  scale_x_continuous(labels = scales::percent) +
  labs(caption = "Note. If mean values are small, %RSD will approach infinity.") +
  xlab(paste("%RSD", y_lab, "- 2Y ROLLING")) # % relative standard deviation
print(plt)
