...
df <- read.csv(file = file.path(dir_data, "school-workforce-in-england_2023", "data", "sickness_absence_teachers_sch.csv"))
names(df) <- gsub("X...", "", names(df), fixed = T)

# read in info about dataset
dict <- read.csv(file = file.path(dir_misc, "meta_absences.csv"))
...
Filename: sickness_absence_teachers_sch.csv  
Geographic levels: School  
Time period: 2014/15 to 2022/23  
Content summary: Full and part time teacher vacancies, temporarily filled posts in state-funded schools in England, by sector and grade. Numbers and rates. School level.  
