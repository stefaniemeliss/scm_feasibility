...
df <- read.csv(file = file.path(dir, "data", "school-workforce-in-england_2023", "data", "vacancies_number_rate_sch_2010_2023.csv"))
names(df) <- gsub("X...", "", names(df), fixed = T)

# read in info about dataset
dict <- read.csv(file = file.path(dir, "misc", "meta_vacancies.csv"))
...
# replace with NAs
df <- df %>%
  mutate(across(matches("rate"), ~na_if(., "x"))) %>% # x = not available - information has not been collected or there are no estimates available at this level of aggregation.
  mutate(across(matches("rate"), ~na_if(., "z"))) %>% # z = not applicable - statistic cannot be produced. For example where a denominator is not available to produce a percentage.
  mutate(across(matches("rate"), ~na_if(., "c"))) %>% # c = confidential - where presentation of data would disclose confidential information
  mutate(across(matches("rate"), ~na_if(., "u"))) # u = low reliability - values of the potentially low quality, for example where values of statistical significance have been calculated.

# make numeric
df[, grepl("rate", names(df))] <- apply(df[, grepl("rate", names(df))], MARGIN = 2, FUN = as.numeric)
...
Filename: vacancies_number_rate_sch_2010_2023.csv  
Geographic levels: School  
Time period: 2010/11 to 2023/24  
Content summary: Full and part time teacher vacancies, temporarily filled posts in state-funded schools in England, by sector and grade. Numbers and rates. Includes school level data.  
