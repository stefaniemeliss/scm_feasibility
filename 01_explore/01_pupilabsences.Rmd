...
df <- read.csv(file = file.path(dir_data, "data_pupilabsences.csv"))
names(df) <- gsub("X...", "", names(df), fixed = T)

# read in info about dataset
dict <- read.csv(file = file.path(dir_misc, "meta_pupilabsences.csv"))
names(dict) <- gsub("X...", "", names(dict), fixed = T)
dict$Variable <- tolower(dict$Variable)
dict <- dict[1:2, ]
...
Pupil absences - school level  
  
Time period: 2010/11 to 2022/23  
Content summary: Percentage of pupil absence in state-funded schools in England, by sector and grade. Numbers and rates. School level.  
...
for (var in vars) {
  ...
  y_lab = dict$Label[dict$Variable == var]
