...
summary <- read.csv(file.path(dir, "02_scm",  "01_treated_schools_examine_dv_preds_out.csv"))
...
# select columns
df <- df[, c("laestab", "time_period", "time_period_str", "school", "pupil_to_qual_teacher_ratio", "fte_avg_age", "pnpupfsm_e")]
...
# select columns
df <- df[, c("laestab", "time_period", "school", "pupil_to_qual_teacher_ratio", "fte_avg_age", "pnpupfsm_e")]

# create data
df_long <- df %>%
  mutate(status = ifelse(laestab == paste(id_treated), id_name, "Donor schools")) %>%
  tidyr::pivot_longer(
    cols = -c(laestab, school, time_period, status),
    names_to = "variable") %>%
  mutate(variable = case_match(variable, 
                               "pupil_to_qual_teacher_ratio" ~ "Outcome",
                               "fte_avg_age" ~ "Teacher age",
...
# check data for NAs
vars <- setdiff(names(df), c("laestab", "time_period", "school"))
...
# # print output
# kbl(obs_pa, caption = paste("Non-NA observations for each variable")) %>%
#   kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T) %>%
#   add_footnote("Bold indicates NA observations for a given year and variable.") %>%
#   {
#     # Apply column_spec to all columns
#     kbl_obj <- .
#     for (i in seq_along(obs_pa)) {
#       if(i == 1) next # skip column containing time_period
#       kbl_obj <- kbl_obj %>%
#         column_spec(i, bold = ifelse(obs_pa[, i] != (length(id_cont)+length(id_treated)), TRUE, FALSE))
#     }
#     kbl_obj
#   } %>%
#   print()
...
for (i in 1:length(list_laestab_treated)) {
  ...
  temp <- merge(
    ...
    sd_filtering(data = df, var = "pupil_to_qual_teacher_ratio", show_kbl = F),
    by = "laestab")
  ...
  # filter by SD crit
  sd_filtering(data = df, perc = 100, var = "pupil_to_qual_teacher_ratio", show_kbl = T)
