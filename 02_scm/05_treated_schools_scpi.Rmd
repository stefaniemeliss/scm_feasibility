...
# Plot the feature values
print(ggplot(data = comparison_raw, aes(x = Year, y = Value, color = Unit, group = Unit)) +
        geom_vline(xintercept = period.pre[length(period.pre)], linetype = "dashed") +
        geom_line() +
        geom_point() +
        facet_wrap(vars(Variable), ncol = 1, scales = "free_y") +
        labs(title = "Feature timeseries without covariate adjustment",
...
if (run_placebo) {
  ...
  # Save results
  write.csv(storegaps, file = file_name, row.names = FALSE)
...
target <- ratio[names(ratio) == id_treated]

# ratio as large as treated unit's ratio
sum(ratio >= target)

# p value
sum(ratio >= target) / length(ratio)
cat("When assigning the intervention at random in the data, the probability of obtaining a post-period RMSPE / pre-period RMSPE ratio as large as the treated school's is ", round(sum(ratio >= target) / length(ratio), digits = 3), ".\n\n", sep = "")
...
# ggplot(data = df_gaps, aes(x = time_period, y = pupil_to_qual_teacher_ratio, group = laestab)) +
