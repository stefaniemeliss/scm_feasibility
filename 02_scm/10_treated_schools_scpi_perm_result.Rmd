...
kbl(df_out[, -c(grep("laestab|school|pre|donor|ill", names(df_out)))], digits = 3, row.names = F, caption = paste(id_name)) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), fixed_thead = T) %>%
  row_spec(which(df_out$prob_thresh5 < .05), bold = T) %>%
  add_footnote(c(
    ...
    paste0("The pooled pre-treatment outcome timeseries mean at the national level is ", round(unique(pooled_mean), 3)," with a pooled timeseries SD at national level of ", round(unique(pooled_sd), 3), "."),
