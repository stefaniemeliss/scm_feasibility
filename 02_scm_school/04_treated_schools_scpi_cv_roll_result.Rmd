...
# declare file name
file_name <- file.path(dir, "02_scm", "interim", paste0("03_treated_schools_scpi_cv_roll_gridsearch_", gsub(" ", "_", id_name), ".csv"))
...
plt <- ggplot(data = tmp_long[tmp_long$roll_w == "Raw IV", ], aes(x = x_position, y = rmspe, group = it)) +
  geom_hline(data = function(y) y %>% group_by(feat_dv, adj, pretreat, region, sd) %>% summarise(sd_treated = unique(sd_treated), .groups = 'drop'),
             aes(yintercept = sd_treated),color = black40, linetype = "dashed") +
  geom_point(aes(colour = method), size = 1) +
  geom_line(aes(colour = method)) +
  scale_x_continuous(
  ...
  ggh4x::facet_grid2(cols = vars(feat_dv, adj), rows = vars(pretreat, region, sd), strip = ggh4x::strip_nested()) +
  ambition_theme +
  theme(strip.text = element_text(size = 10),
        legend.title = element_text(size = 10),
        legend.text = element_text(size = 10),
