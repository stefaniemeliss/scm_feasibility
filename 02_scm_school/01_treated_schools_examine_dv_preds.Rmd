...
ts_desc$out_sd <- apply(df[, grepl("pup|fte", names(df))], MARGIN = 2, FUN = function(x){sum(is_outlier_3sd(x), na.rm = T)})
ts_desc$out_iqr <- apply(df[, grepl("pup|fte", names(df))], MARGIN = 2, FUN = function(x){sum(is_outlier_iqr(x), na.rm = T)})

if (nrow(df) >= 3) {
  
  # compute snr
  ts_desc$snr <- apply(df[, grepl("pup|fte", names(df))], MARGIN = 2, FUN = function(x){
