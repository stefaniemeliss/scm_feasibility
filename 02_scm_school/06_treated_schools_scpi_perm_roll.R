...
for (i in 1:length(info)) {
  ...
  for (decrease in increments) {
    ...
    for (j in 1:length(preproc)) {
      ...
      roll.vars <- c("pupil_to_qual_teacher_ratio", "pnpupfsm_e", "fte_avg_age")
      ...
      if (run_placebo) {
        ...
        # determine output filename
        file_name <- file.path(dir, "02_scm", paste0(file_stem, "_" , gsub(" ", "_", id_name), "_", setting, "_decrease_", decrease, ".csv"))
