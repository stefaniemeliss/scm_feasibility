...
file_name <- file.path(dir, "03_scm_mat", paste0(file_stem, "_mixed.csv"))
...
```{r, echo=F, results='asis', message=FALSE, eval = T, fig.height=15, fig.width=15}
...
```{r data_prep_sec, include=F}
# Define target regions for filtering the donor pool
regions <- c("Yorkshire and the Humber", "North West")

all_but_secondary = unique(groups$phaseofeducation_name)[!grepl("econdary", unique(groups$phaseofeducation_name))]

# process data at MAT level
process_data_scm_mat(uid_treated = uid_treated, target_regions = regions, filter_phase = all_but_secondary,
                    swf_filter = "! time_period %in% c(201011, 201112)")
```

## Treated MAT  

```{r check_treated_sec, echo=FALSE, results='asis', eval=T}
# add info about schools
groups %>% 
  filter(laestab %in% df$laestab[df$group_uid == uid_treated]) %>%
  select(-c(urn, establishmentnumber, la_code, group_uid, group_id, establishmenttypegroup_name, group_name, group_closed_date, date_left_group, laestab, establishment_closedate, reasonestablishmentclosed_name)) %>%
...
```{r check_donors_sec, echo=FALSE, results='asis', eval=T}
knitr::opts_chunk$set(echo = TRUE)
# format information on MATs
MATs$multiple_phases <- grepl(" | ", MATs$phase, fixed = T)
MATs <- create_element_columns(data = MATs, column_name = "phase", drop = F)
...
file_name <- file.path(dir, "03_scm_mat", paste0(file_stem, "_secondary.csv"))
...
if (run_gridsearch) {
  ...
  results <- do.call(rbind, lapply(1:nrow(filter_grid[, ]), function(i) {
    ...
    result <- tryCatch({
      process_data_scm_mat(uid_treated = uid_treated, 
                           target_regions = unlist(filters$regions), 
                           filter_phase = all_but_secondary,
                           min_years_obs = filters$min_years_obs,
                           min_schools_per_mat = filters$min_schools_per_mat,
                           min_schools_per_timeperiod = filters$min_schools_per_timeperiod,
                           swf_filter = swf_filter_param)
...
tmp$NW <- ifelse(grepl("North West", tmp$regions), "Region: YS&H + NW", "Region: YS&H")
tmp$excl_nw<- ifelse(tmp$exclude_northwest, "excl. NW based mat", "incl. NW based mat")
tmp$years <- ifelse(grepl("201314", tmp$swf_filter), "excl. 10/11 - 13/14", "excl. 10/11 + 11/12")

ggplot(data = tmp, aes(x = treated_incl, y = n_donors)) +
  geom_point() +
  facet_grid(rows = vars(NW, excl_nw, years), cols = vars(min_size, min_it, min_t0))
...
```{r data_prep_prim, include=F}
# Define target regions for filtering the donor pool
regions <- c("Yorkshire and the Humber", "North West")

all_but_primary = unique(groups$phaseofeducation_name)[!grepl("imary", unique(groups$phaseofeducation_name))]

# process data at MAT level
process_data_scm_mat(uid_treated = uid_treated, target_regions = regions, filter_phase = all_but_primary,
                    swf_filter = "! time_period %in% c(201011, 201112, 201213, 201314)")
```

## Treated MAT  

```{r check_treated_prim, echo=FALSE, results='asis', eval=T}
# add info about schools
groups %>% 
  filter(laestab %in% df$laestab[df$group_uid == uid_treated]) %>%
  select(-c(urn, establishmentnumber, la_code, group_uid, group_id, establishmenttypegroup_name, group_name, group_closed_date, date_left_group, laestab, establishment_closedate, reasonestablishmentclosed_name)) %>%
...
```{r check_donors_prim, echo=FALSE, results='asis', eval=T}
knitr::opts_chunk$set(echo = TRUE)
# format information on MATs
MATs$multiple_phases <- grepl(" | ", MATs$phase, fixed = T)
MATs <- create_element_columns(data = MATs, column_name = "phase", drop = F)
...
file_name <- file.path(dir, "03_scm_mat", paste0(file_stem, "_primary.csv"))
...
tmp$NW <- ifelse(grepl("North West", tmp$regions), "Region: YS&H + NW", "Region: YS&H")
tmp$excl_nw<- ifelse(tmp$exclude_northwest, "excl. NW based mat", "incl. NW based mat")
tmp$years <- ifelse(grepl("201415", tmp$swf_filter), "excl. 10/11 - 14/15", "excl. 10/11 - 13/14")
