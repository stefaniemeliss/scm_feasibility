...
df_avg_raw <- df_sim_raw %>% # APPLY EFFECTS AT MAT LEVEL
  ...
  mutate(
    ...
    !!sym(dv) := ifelse(is.na(!!sym(dv)), sim, !!sym(dv))
    ) %>%
...
if (run_placebo) {
  ...
  units <- unique(df_avg_raw$group_uid)
  ...
  for(i in 1:length(units)){
    ...
    unit.co <- unique(df_avg_raw$group_uid[df_avg_raw$group_uid != unit])
  ...
  for (y in 1:length(unique(results_fit$period.pre))) {
    ...
    if (nrow(results_fit[results_fit$period.pre == years & results_fit$it %in% it, ]) > 0) {
      ...
      <<est>>
        ...
        file_name <- file.path(dir, "03_scm_mat", "interim", paste0(file_stem, "_gaps_" , tolower(phase), "_it_", sprintf("%03d", ii), "_cv.csv"))
      run_placebo <- T
      <<placebo>>
      ...
      cat("\n\n")
      # determine output filename
        file_name <- file.path(dir, "03_scm_mat", "interim", paste0(file_stem, "_gaps_" , tolower(phase), "_it_", sprintf("%03d", ii), "_sim.csv"))
      #run_placebo <- F
      #<<placebo>>
