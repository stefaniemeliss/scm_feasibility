...
```{r, echo=FALSE, results='asis'}
# update laestab list
list_laestab <- unique(summary$laestab[summary$n >= 4])
...
z <- swf %>%
  filter(laestab %in% list_laestab) %>%
  select(time_period, laestab, pupil_to_qual_teacher_ratio, fte_avg_age)

# Filter to create predictor dataset
x <- pup %>% 
  filter(laestab %in% list_laestab) %>%
  select(time_period, laestab, pnpupfsm_e)
...
# df %>% group_by(laestab) %>% summarise(n = n()) # check that there are more than 4 obs per school

# compute MAT timeseries average
tmp <- df %>% 
  group_by(time_period) %>% 
  summarise(
    uid_treated = uid_treated,
    pupil_to_qual_teacher_ratio = mean(pupil_to_qual_teacher_ratio),
    fte_avg_age = mean(fte_avg_age),
    pnpupfsm_e = mean(pnpupfsm_e),
    n = n()
  )
...
df_long <- tmp %>%
  tidyr::pivot_longer(
    cols = -c(uid_treated, n, time_period),
    names_to = "variable") %>%
  mutate(category = case_match(variable, 
                               "pupil_to_qual_teacher_ratio" ~ "Outcome",
                               "fte_avg_age" ~ "Teacher age",
...
# plot timeseries for each variable
plt <- ggplot(data = df_long, aes(x = time_period_str, y = value, group = uid_treated)) +
  geom_line(col = coral) + 
...
# compute timeseries descriptives
ts_desc <- apply(tmp[, grepl("pup|fte", names(tmp))], MARGIN = 2, FUN = function(x){psych::describe(x, fast = T)})

# combine to df
ts_desc <- do.call("rbind",ts_desc)
ts_desc$vars <- NULL

ts_desc$out_count <- apply(tmp[, grepl("pup|fte", names(tmp))], MARGIN = 2, FUN = function(x){sum(is_outlier_3sd(x))})
...
# print to markdown
out %>% 
  mutate(Variable = factor(Variable, levels = c("Outcome (raw)", "FSM (raw)", "Age (raw)"))) %>%
  arrange(Variable, sd) %>%
